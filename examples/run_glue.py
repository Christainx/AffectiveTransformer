...
#     fp_pred = open( str( global_step ) + '_pred.txt', 'w', encoding = 'utf8' )
#     fp_true = open( str( global_step ) + '_true.txt', 'w', encoding = 'utf8' )
...
#             fp_true.writelines( str( item ) + '\n' )
        ...
        with open(output_eval_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                ...
                writer.write("%s = %s\n" % (key, str(result[key])))
                LOG_FP.writelines( key + ' = ' + str( result[key] ) + '\n' )
                LOG_FP.flush()
...
if __name__ == "__main__":
    for item in CUR_DATAS:
        ...
        print( CUR_DATA )
        global LOG_FP
        LOG_FP = open( MODEL_NAME + '_' + CUR_DATA + '_' + AFFECTIVE_RES + '.txt', 'w', encoding = 'utf8' )

        global epainfos
        epainfos = EPAInfo()

        if AFFECTIVE_RES == 'EPA.pkl':
            rng = np.arange ( 0.5, 2.0, 0.2 )
        else:
            rng = np.arange ( 0.5, 4.0, 0.5 )
        for al in rng:
            epainfos.ALPHA = float( al )
            print( al )
            LOG_FP.writelines( str( al ) + '\n' )

            for weightgroup in COMPACT_ARRAY:
                we = weightgroup[0]
                epainfos.WEIGHT_E = we 
                print( we )

                wp = weightgroup[1]
                epainfos.WEIGHT_P = float( wp )
                print( wp )
                
                wa = float( 1.0 ) - we - wp 
                epainfos.WEIGHT_A = np.float( wa )
                print( wa )

                LOG_FP.writelines( '    ' + str( we ) + ' ' + str( wp ) + ' ' + str( wa ) + '\n' )
