...
class PreTrainedTokenizer(object):
    ...
    def tokenize( self, text, epa_infos, **kwargs ):
        ...
        def split_on_tokens( tok_list, text, epa_infos ):
            if not text:
                return []
            if not tok_list:
...
#                     else [token] for token in tokenized_text), [])
            ...
            for token in tokenized_text:
                if token not in self.added_tokens_encoder and token not in self.all_special_tokens:
                    temp, epa_vector = self._tokenize_withepa( token, epa_infos, ** kwargs )
                else:
                    temp = [token]

            return temp, epa_vector

        added_tokens = list(self.added_tokens_encoder.keys()) + self.all_special_tokens
        tokenized_text, epa_vector = split_on_tokens( added_tokens, text, epa_infos )
        return tokenized_text, epa_vector
