...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaModel(config)
        self.classifier = RobertaClassificationEPA( config )

        self.meanpooler = BertMeanPooler( config )

    def forward_orig( self, input_ids, token_type_ids = None, attention_mask = None, labels = None,
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
                position_ids = None, head_mask = None, epa_matrix = None ):
        outputs = self.roberta(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask)
        sequence_output = outputs[0]

        expsize = sequence_output.size()
        expsize = ( 1, 1, expsize[-1] )
        epa_matrix = epa_matrix.unsqueeze( 2 )
        epa_matrix_temp = epa_matrix.repeat( expsize )
        sequence_output = sequence_output.mul( epa_matrix )
        sequence_output = self.meanpooler( sequence_output )

#         pooled_output_bert = outputs[1]
#         pooled_output = torch.cat( ( pooled_output_bert, pooled_output ), dim = 1 )
#         pooled_output = pooled_output_bert * 0.5 + pooled_output * 0.5
...
class RobertaClassificationEPA( nn.Module ):
    """Head for sentence-level classification tasks."""

    def __init__( self, config ):
        super( RobertaClassificationEPA, self ).__init__()
        self.dense = nn.Linear( config.hidden_size, config.hidden_size )
